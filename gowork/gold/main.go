...
func setConf() {
	...
	//mysql密码
	config.MYSQL_OUTPUT.Password = "88888" //
	config.MYSQL_OUTPUT.Table = "gold"
}
func Run() {
	// 开启最大核心数运行
	...
	outputflag := flag.String("output", LogicApp.GetOutputLib()[1], outputlib)
	// 并发协程数
	goroutineflag := flag.Uint("go", 20, "   【并发协程】   {1~99999}\r\n")
	// 分批输出
	dockerflag := flag.Uint("docker", 1000, "   【分批输出】   每 {1~5000000} 条数据输出一次\r\n")
	// 暂停时间
	...
	// 创建蜘蛛队列
	sps := []*spider.Spider{}
	sps = append(sps, LogicApp.GetSpiderByName("Gold"))
	//if *spiderflag == "" {
