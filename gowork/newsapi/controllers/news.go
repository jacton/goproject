...
// @Param	direction		query 	string	false		"The direction for news 0:up 1:down"
...
func (u *NewsController) GetNews() {
	...
	direction := u.GetString("direction")
	jsdata := models.GetNews(gid, limit, direction)
	u.Data["json"] = jsdata
	u.ServeJSON()
}
...
func (u *NewsController) GetNewsInfo() {
	...
	limit := u.GetString("limit")
	direction := u.GetString("direction")
	jsdata := models.GetNewsInfo(strtype, strdatetime, limit, direction)
	u.Data["json"] = jsdata
	u.ServeJSON()
}
